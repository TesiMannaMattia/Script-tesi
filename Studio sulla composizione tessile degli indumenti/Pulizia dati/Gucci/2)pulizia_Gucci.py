...
#df = pd.read_csv('/Users/mattia/Library/Mobile Documents/com~apple~CloudDocs/Tesi/Dati/Dati grezzi (NON TOCCARE!)/ARMANI.csv',usecols=['Sesso', 'Categoria', 'Sotto_categoria', 'Marca','Articolo', 'Prezzo', 'Materiali', 'Colore', 'Codice_prodotto', 'Link'])
...
for i in range(0,len(df)):
  ...
  if m in arancione:
    ...
    if m not in assenti :
        assenti.append(m)  
        n+= 1 
        with open("/Users/mattia/Desktop/riepilogo_colori_Gucci.txt","a") as f:
...
for i in range(0,len(m)):
  ...
  for j in range(0,len(ma)):
    ...
    elif ma[j]=="Polyamide" or  ma[j]=="Polyimide" or ma[j]=="Polyimide" or ma[j]=="Poliammide_e":
        ma[j]="Poliammide"
    ...
    elif ma[j]=="Pelle_di_capra" or ma[j]=="Pelle_bovina" or  ma[j]=="Pelle_di_vitello" or  ma[j]=="Mucca" or ma[j]=="Cuoio" or ma[j]=="Calf" or  ma[j]=="Bovine_leather" or  ma[j]=="Goat_skin"or ma[j]=="Skin_leather" or ma[j]=="Lambskin" or ma[j]=="Pelle_di_agnello" or ma[j]=="Pelle_di_bovino":
        ma[j]="Pelle"
     
    elif ma[j]=="Linen":
        ma[j]="Lino"
        
    elif ma[j]=="Metallic_fibre" or  ma[j]=="Ottone" or  ma[j]=="Metallica" or  ma[j]=="Fibra_metallizzata"  or ma[j]=="Metallo":
    ...
    elif ma[j]=="Other_fibres" or ma[j]=="Fibre_non_specificate" or ma[j]=="Fibre":
        ma[j]="Altre_fibre"
        
    elif ma[j]=="Dettaglio" or  ma[j]=="Imbottitura" or  ma[j]=="Italia" or  ma[j]=="Moldavia" or ma[j]=="Spagna" or ma[j]=="Sud_africa":
        da_eliminare.add(ma[j])
    
  ma=set(ma)
  ma=list(ma.difference(da_eliminare))
...
dati.to_csv('/Users/mattia/Library/Mobile Documents/com~apple~CloudDocs/Tesi/Dati/Dati puliti/pulito_Gucci.csv')

#dati.to_csv('/Users/mattia/desktop/prova_gucci.csv')
